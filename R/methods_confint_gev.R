...
#'
#' Methods for objects of class `"confint_gev"` returned from [`confint.evmiss`].
#' @param x An object inheriting from class `"confint_gev"`, an object returned
#'   after a call to [`confint.evmiss`].
...
#'   argument `parm` in [`confint.evmiss`]. These columns are labelled as
...
#'   the log-likelihood in the call to [`confint.evmiss`] are included, so
...
#' @section Examples: See [`evmiss_methods`].
#' @seealso [`gev_mle`] and [`evmiss_methods`].
...
plot.confint_gev <- function(x, parm = c("mu", "sigma", "xi"), add = TRUE,
                             digits = 2, ...) {
  # Check that confint.evmiss() was called with profile = TRUE
  if (is.null(attr(x, "for_plot"))) {
    stop("Call confint.evmiss() with profile = TRUE")
  }
  # Check that parm was included in parm in the call to confint.evmiss()
  parameters_in_x <- attr(x, "dimnames")[[1]]
  if (any(!is.element(parm, parameters_in_x))) {
    stop(parm, " was not included in ''parm'' in the call to confint.evmiss.")
  }
  parm <- match.arg(parm)
  if (parm == "mu") {
