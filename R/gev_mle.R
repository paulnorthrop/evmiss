...
#' \eqn{G(x)}. Let \eqn{n_i} be the number of missing values in block \eqn{i}
...
#'
#' The class of the returned object is `c("evmiss", "mle", "list")`.
#'
#' Objects inheriting from class `"evmiss"` have `coef`, `logLik`, `nobs`,
...
gev_mle <- function(data, block_length, block, adjust = TRUE, discard = 0,
  ...
  if (inherits(data, "block_maxima") && inherits(data, "evmiss")) {
    maxima_notNA <- data
  ...
  class(fit) <- c("evmiss", "mle", class(fit))
  return(fit)
}
