#' Methods for objects of class `"evmiss_sim_study_2"`
#'
#' Methods for objects of class `"evmiss_sim_study_2"` returned from
#'   [`sim_study_2`].
#' @param x,object An object inheriting from class `"evmiss_sim_study_2"`, a
#'   result of a call to the function [`sim_study_2`]. For
#'   `plot.evmiss_sim_study_2`, further arguments to be passed to
...
#'
#' * `plot.evmiss_sim_study_2`: to pass graphical parameters to the graphical
#'   functions [`plot`][graphics::plot], [`hist`][graphics::hist].
#' * `print.summary.evmiss_sim_study_2`: if `quiet = FALSE`, to pass arguments
#'   to [`print`][base::print], such as `digits`.
#' * `tab.evmiss_sim_study_2`: to pass arguments to
#'   `summary.evmiss_sim_study_2`, such as `vsfull`.
#' @details
#'
...
#' available is to look at the examples in** [`sim_study_2`].
#'
...
#' `summary.evmiss_sim_study_2` then the first row relates to the `"full"`
...
#'
#' @seealso [`sim_study_2`].
#' @name sim_study_2_methods
#' @section Examples: See [`sim_study_2`].
NULL
## NULL
...
#'   external to the call to `plot.evmiss_sim_study_2`.
...
#'
#' @export
plot.evmiss_sim_study_2 <- function(x, what = c("return", "mu", "sigma", "xi",
                                              "all"),
...
#' summary method for objects of class `"sim_study_2"`
...
#'   `c("summary.evmiss_sim_study_2", "list")`, containing the original function
#'   call to `summary.evmiss_sim_study_2` and the following components, depending
#'   on `what` and `vsfull`.
#'
...
#'
#'   The object is printed by [`print.summary.evmiss_sim_study_2`]. The rows
...
#'
#' @export
summary.evmiss_sim_study_2 <- function(object, ..., what = c("all", "return"),
  ...
  mle5 <- object$parameters[13:15, ]
  mle6 <- object$parameters[16:18, ]
  # Each of mle1, mle2, mle3, mle4, mle5 and mle6 is a 3 by nsim matrix with rows
  ...
  if (vsfull) {
    ...
    compare5 <- penult_parameters
    compare6 <- penult_parameters
  }
  ...
  if (log) {
    ...
    log_compare5[2] <- log(compare5[2])
    log_compare6[2] <- log(compare6[2])
    log_mle1 <- mle1
    log_mle2 <- mle2
    log_mle3 <- mle3
    log_mle4 <- mle4
    ...
    log_mle5[2, ] <- log(log_mle5[2, ])
    log_mle6[2, ] <- log(log_mle6[2, ])
    ...
    weight1_gev <- apply(log_mle5 - log_compare5, 1, FUN = statistics)
    weight2_gev <- apply(log_mle6 - log_compare6, 1, FUN = statistics)
  } else {
    ...
    if (meanSE && !vsfull) {
      ...
      se5 <- object$ses[13:15, ]
      se6 <- object$ses[16:18, ]
      ...
      discard_gev <- rbind(discard_gev, meanSE = apply(se4, 1, FUN = mean,
                                                       na.rm = TRUE))
      weight1_gev <- rbind(weight1_gev, meanSE = apply(se5, 1, FUN = mean,
                                                       na.rm = TRUE))
      weight2_gev <- rbind(weight2_gev, meanSE = apply(se6, 1, FUN = mean,
                                                       na.rm = TRUE))
    }
  }
  ...
  weight1_rl <- as.matrix(sapply(1:length(return_period), FUN = rl_function,
                                 mles = mle5, log_return = log_return))
  weight2_rl <- as.matrix(sapply(1:length(return_period), FUN = rl_function,
                                 mles = mle6, log_return = log_return))
  ...
  if (meanSE && !vsfull) {
    ...
    rows_weight1 <- grepl("weight1", rownames(object$rl_ses))
    rows_weight2 <- grepl("weight2", rownames(object$rl_ses))
    ...
    rl_se5 <- object$rl_ses[rows_weight1, ]
    rl_se6 <- object$rl_ses[rows_weight2, ]
    ...
    discard_rl <- rbind(discard_rl,
                        meanSE = apply(rl_se4[which_rows, , drop = FALSE], 1,
                                                   FUN = mean, na.rm = TRUE))
    weight1_rl <- rbind(weight1_rl,
                      meanSE = apply(rl_se5[which_rows, , drop = FALSE], 1,
                                     FUN = mean, na.rm = TRUE))
    weight2_rl <- rbind(weight2_rl,
                        meanSE = apply(rl_se6[which_rows, , drop = FALSE], 1,
                                       FUN = mean, na.rm = TRUE))
  }
  ...
  if (what == "all") {
    ...
    res$weight1_rl <- weight1_rl
    res$weight2_rl <- weight2_rl
  }
  res$vsfull <- vsfull
  class(res) <- c("summary.evmiss_sim_study_2", class(res))
  return(res)
}

# ============================ print.summary method ========================= #

#' Print method for objects of class `"summary.evmiss_sim_study_2"`
#'
#' @rdname sim_study_2_methods
#' @param return_period Either a numeric scalar (for `plot.evmiss_sim_study_2`)
#'   or a numeric vector (for `summary.evmiss_sim_study_2` or
  ...
  if (any(grepl("gev", names(x)))) {
    ...
    if (x$vsfull) {
      mat <- rbind(x$adjust_rl, x$naive_rl, x$discard_rl,
                   x$weight1_rl, x$weight2_rl)
      mat <- mat[order(c(seq_len(nrow(x$adjust_rl)),
                         seq_len(nrow(x$naive_rl)),
                         seq_len(nrow(x$discard_rl)),
                         seq_len(nrow(x$weight1_rl)),
                         seq_len(nrow(x$weight2_rl)))), , drop = FALSE]
      if (is.null(rownames(mat))) {
        rownames(mat) <- rep(paste0("stat", 1:nstats), each = 3)
      }
