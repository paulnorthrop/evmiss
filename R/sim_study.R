...
#'
#' # All parameters, for each approach
...
#' plot(res, what = "all", distn = "joint", approach = c("full", "naive"))
#' plot(res, what = "all", distn = "joint", approach = c("adjust", "naive"))
#'
#' ### Summaries
#'
#' summary(res)
#' summary(res, vsfull = TRUE)
...
#' summary(res, what = "return", return_period = c(100, 1000))
#' @export
sim_study <- function(nsim, return_periods, discard = 25, level = 0.95,
  ...
  replication_fun <- function(x, return_periods) {
    ...
    # Return everything
    val <- c(mle1, mle2, mle3, mle4, se1, se2, se3, se4,
             lower_sym_1, lower_sym_2, lower_sym_3, lower_sym_4,
             upper_sym_1, upper_sym_2, upper_sym_3, upper_sym_4)
    if (profile) {
      val <- c(val,
  ...
  method_name <- c("full", "adjust", "naive", "discard")
