...
#'
#' @param x An object inheriting from class `evmiss` returned from [`gev_mle`].
...
#' @return An object with class `c("return_level", "numeric", "evmiss")`.
...
gev_return <- function(x, m = 100, npy = 1) {
  ...
  class(return_levels) <- c("return_level", class(return_levels), "evmiss")
