#' Methods for objects of class `"evmiss"`
#'
#' Methods for objects of class `"evmiss"` returned from [`gev_mle`].
#' @param object An object inheriting from class `"evmiss"`, a result of a
#'   call to [`gev_mle`].
#' @param x An object returned by `summary.evmiss`.
#' @param ... Further arguments. Only used in the following cases.
#'
#'   * `plot.evmiss`: to pass graphical parameters to the graphical functions
...
#'   * `print.summary.evmiss`: to pass arguments to [`print`].
#' @details The plots produced by `plot.evmiss` are of a similar form to the
...
#'   produced by `plot.evmiss` is `profile = TRUE`, which uses [`gev_return`]
...
#' @return `coef.evmiss`: a numeric vector of length 3 with names
#'   `c("mu", "sigma", "xi")`.  The MLEs of the parameters \eqn{\mu},
#'   \eqn{\sigma} and \eqn{\xi}.
#'
#'   `vcov.evmiss`: a \eqn{3 \times 3}{3 x 3} matrix with row and column names
#'   `c("mu", "sigma", "xi")`. The estimated variance-covariance matrix for the
#'   model parameters \eqn{\mu}, \eqn{\sigma} and \eqn{\xi}.
#'
#'   `nobs.evmiss`: a numeric scalar. The number of maxima used in the model
#'   fit.
#'
#'   `logLik.evmiss`: an object of class \code{"logLik"}: a numeric scalar
...
#'
#'   `summary.evmiss`: an object with class `"summary.evmiss"` containing the
#'     original function call and a matrix of estimates and estimated standard
#'     errors with row names `c("mu", "sigma", "xi")`.  The object is printed
#'     by [`print.summary.evmiss`].
#'
#'   `print.summary.evmiss`: the argument `x` is returned, invisibly.
#'
#'   `confint.evmiss`: an object of class `c("confint_gev", "evmiss")`.
...
#' Extract model coefficients method for objects of class `"evmiss"`
#'
#' @rdname evmiss_methods
#' @export
coef.evmiss <- function(object, ...) {
  return(object$par)
}

# ================================ vcov.evmiss =============================== #

#' Calculate the variance-covariance matrix for an object of class `"evmiss"`
#'
#' @rdname evmiss_methods
#' @export
vcov.evmiss <- function(object, ...) {
  vc <- object$vcov
...
#' Extract the number of observations for objects of class `"evmiss"`
#'
#' @rdname evmiss_methods
#' @export
nobs.evmiss <- function(object, ...) {
  return(length(object$maxima))
}

# ================================ logLik.evmiss ============================= #

#' Extract log-likelihood for objects of class `"evmiss"`
#'
#' @rdname evmiss_methods
#' @export
logLik.evmiss <- function(object, ...) {
  val <- object$loglik
...
#' @rdname evmiss_methods
#' @export
summary.evmiss <- function(object, digits = max(3, getOption("digits") - 3L),
                           ...) {
  res <- list()
  ...
  class(res) <- "summary.evmiss"
  return(res)
}

# ============================ print.summary.evmiss ========================== #

#' Print method for objects of class `"summary.evmiss"`
#'
#' @rdname evmiss_methods
#' @export
print.summary.evmiss <- function(x, ...) {
...
#' Confidence intervals for \code{"evmiss"} objects
#'
...
#'     faster still.
#' @details For `confint.evmiss`, the default, `epsilon = -1`, should work well
...
#'   `plot.evmiss` are adjusted for the number of non-missing observations
...
#' @rdname evmiss_methods
#' @export
plot.evmiss <- function(x, adjust = TRUE,
