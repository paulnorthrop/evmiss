...
fit1 <- evmiss::gev_mle(BloomsburyOzone$Ozone, block = BloomsburyOzone$Year)
fit2 <- evmiss::gev_mle(BloomsburyOzone$Ozone, block = BloomsburyOzone$Year,
                        adjust = FALSE)
coef(fit1)
coef(fit2)

# Fit using the data frame of maxima
fit1 <- evmiss::gev_mle(BloomsburyOzoneMaxima)
fit2 <- evmiss::gev_mle(BloomsburyOzoneMaxima, adjust = FALSE)
coef(fit1)
coef(fit2)
